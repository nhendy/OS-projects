...
// Definition: a "jiffy" is defined as the minimum clock resolution.  For
// instance,
// if timer interrupts are set to go off every 1 millisecond, then 1 jiffy = 1
// milliseond.

static int curtime = 0;  // The number of "jiffies" that have passed
static int clock_resolution =
    CLOCK_DEFAULT_RESOLUTION;  // Number of microseconds in one "jiffy"
static int clock_running = 0;  // Flag to enable starting/stopping clock
static int last_trigger_jiffies =
    0;  // Keeps track of last time we triggered ProcessSchedule
...
//-------------------------------------------------------------
inline void ClkSetResolution(int new_res) { clock_resolution = new_res; }

//-------------------------------------------------------------
// ClkGetResolution returns the current clock resolution in
// microseconds per jiffy.
//-------------------------------------------------------------
inline int ClkGetResolution() { return clock_resolution; }
...
//-------------------------------------------------------------
inline int ClkGetCurJiffies() { return curtime; }

//-------------------------------------------------------------
// ClkResetProcess resets the process jiffies counter
//-------------------------------------------------------------
void ClkResetProcess() { last_trigger_jiffies = curtime; }
