...
int ProcessFork(VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  uint32 offset;  // Used in parsing command line argument strings, holds offset
                  // (in bytes) from
  ...
  // fill in the
  // initial stack frame that will be loaded for this PCB when it gets switched
  // in by
  ...
  if (isUser) {
    ...
    // Compute the addresses in user space of where each string for the command
    // line arguments
    ...
    for (argc = 0; argc < MAX_ARGS; argc++) {
      ...
      // The address can be found as the current stack pointer (which points to
      // the start of
    ...
    // array
    // of uint32's will be copied onto the stack.  We'll move the stack pointer
    // by
    ...
    // Now copy the actual memory.  Remember that stacks grow down from the top
    // of memory, so
    // we need to move the stack pointer first, then do the copy.  The "+2", as
    // before, is
