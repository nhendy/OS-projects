...
int GetHeapParent(int idx, int heap_size) {
  ...
  if (parent_idx < heap_size) return parent_idx;
  return -1;
}
...
void InitHeap(PCB *pcb) {
  int heap_size = sizeof(pcb->heap) / sizeof(pcb->heap[0]);
...
void ProcessModuleInit() {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    if (AQueueInsertFirst(&freepcbs, pcbs[i].l) != QUEUE_SUCCESS) {
      printf(
          "FATAL ERROR: could not insert PCB link into queue in "
          "ProcessModuleInit!\n");
      exitsim();
    }
  }
...
void ProcessFreeResources(PCB *pcb) {
  ...
  if (AQueueInsertLast(&freepcbs, pcb->l) != QUEUE_SUCCESS) {
    printf(
        "FATAL ERROR: could not insert PCB link into freepcbs queue in "
        "ProcessFreeResources!\n");
    exitsim();
  }
...
void ProcessSchedule() {
  ...
  // Move the front of the queue to the end if currentPCB is not on sleep
  // queue.
  ...
  while (!AQueueEmpty(&zombieQueue)) {
    ...
    if (AQueueRemove(&(pcb->l)) != QUEUE_SUCCESS) {
      printf(
          "FATAL ERROR: could not remove zombie process from zombieQueue in "
          "ProcessSchedule!\n");
      exitsim();
    }
...
void ProcessWakeup(PCB *wakeup) {
  ...
  if (AQueueRemove(&(wakeup->l)) != QUEUE_SUCCESS) {
    printf(
        "FATAL ERROR: could not remove wakeup PCB from waitQueue in "
        "ProcessWakeup!\n");
    exitsim();
  }
  ...
  if (AQueueInsertLast(&runQueue, wakeup->l) != QUEUE_SUCCESS) {
    printf(
        "FATAL ERROR: could not insert link into runQueue in "
        "ProcessWakeup!\n");
    exitsim();
  }
}
...
void ProcessDestroy(PCB *pcb) {
  ...
  if ((pcb->l = AQueueAllocLink(pcb)) == NULL) {
    printf(
        "FATAL ERROR: could not get link for zombie PCB in "
        "ProcessDestroy!\n");
    exitsim();
  }
...
int ProcessFork(VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  uint32 initial_user_params[MAX_ARGS + 2];  // Initial memory for user
  ...
  uint32 offset;    // Used in parsing command line argument strings, holds
                    // offset
                    // (in bytes) from
  ...
  // fill in the
  // initial stack frame that will be loaded for this PCB when it gets
  // switched
  // in by
  ...
  if (isUser) {
    ...
    // Copy the entire set of strings of command line parameters onto the user
    // stack.
    // The "param" variable is a pointer to the start of a sequenial set of
    // strings,
    // each ending with its own '\0' character.  The final "string" of the
    // sequence
    // must be an empty string to indicate that the sequence is done.  Since
    // we
    // can't figure out how long the set of strings actually is in this
    // scenario,
    // we have to copy the maximum possible string length and parse things
    // manually.
    stackframe[PROCESS_STACK_USER_STACKPOINTER] -= SIZE_ARG_BUFF;
    MemoryCopySystemToUser(pcb, (char *)param,
                           (char *)stackframe[PROCESS_STACK_USER_STACKPOINTER],
                           SIZE_ARG_BUFF);

    // Now that the main string is copied into the user space, we need to
    // setup
    // argv as an array of pointers into that string, and argc as the total
    // number of arguments found in the string.  The first call to
    // get_argument
    // should return 0 as the offset of the first string.
    offset = get_argument((char *)param);

    // Compute the addresses in user space of where each string for the
    // command
    // line arguments
    // begins.  These addresses make up the argv array.
    for (argc = 0; argc < MAX_ARGS; argc++) {
      // The "+2" is because initial_user_params[0] is argc, and
      // initial_user_params[1] is argv.
      // The address can be found as the current stack pointer (which points
      // to
      // the start of
      // the params list) plus the byte offset of the parameter from the
      // beginning of
      // the list of parameters.
      initial_user_params[argc + 2] =
          stackframe[PROCESS_STACK_USER_STACKPOINTER] + offset;
      offset = get_argument(NULL);
      if (offset == 0) {
        initial_user_params[argc + 2 + 1] =
            0;  // last entry should be a null value
        break;
      }
    }
    // argc is currently the index of the last command line argument.  We need
    // it to instead
    ...
    // Compute where initial_user_params[3] will be copied in user space as
    // the
    // base of the array of string addresses.  The entire initial_user_params
    // array
    // of uint32's will be copied onto the stack.  We'll move the stack
    // pointer
    // by
    // the necessary amount, then start copying the array.  Therefore,
    // initial_user_params[3]
    // will reside at the current stack pointer value minus the number of
    // command line
    // arguments (argc).
    initial_user_params[1] =
        stackframe[PROCESS_STACK_USER_STACKPOINTER] - (argc * sizeof(uint32));

    // Now copy the actual memory.  Remember that stacks grow down from the
    // top
    // of memory, so
    // we need to move the stack pointer first, then do the copy.  The "+2",
    // as
    // before, is
    // because initial_user_params[0] is argc, and initial_user_params[1] is
    // argv.
    ...
    MemoryCopySystemToUser(
        pcb, (char *)initial_user_params,
        (char *)(stackframe[PROCESS_STACK_USER_STACKPOINTER]),
        initial_user_params_bytes);
    ...
    // Don't worry about messing with any code here for kernel processes
    // because
...
void main(int argc, char *argv[]) {
  ...
  if (userprog != (char *)0) {
    ...
    for (i = 0; i < argc - base; i++) {
      // At each argv address, copy the string into allargs, including the
      // '\0'
