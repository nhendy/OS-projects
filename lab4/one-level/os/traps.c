...
uint32 GetUintFromTrapArg(uint32 *trapArgs, int sysmode) {
  uint32 arg;
  if (!sysmode) {
    MemoryCopyUserToSystem(currentPCB, (char *)trapArgs, (char *)&arg,
                           sizeof(arg));
  } else {
...
static void TrapProcessCreateHandler(uint32 *trapArgs, int sysmode) {
  ...
  if (!sysmode) {
    ...
    for (i = 0; i < MAX_ARGS; i++) {
      ...
      // its value
      MemoryCopyUserToSystem(currentPCB, (char *)(trapArgs + 1 + i),
                             (char *)&userarg, sizeof(char *));
      ...
      for (j = 0; j < SIZE_ARG_BUFF; j++) {
        MemoryCopyUserToSystem(currentPCB, (char *)(userarg + j),
                               (char *)&(allargs[allargs_position]),
                               sizeof(char));
...
static void TrapPrintfHandler(uint32 *trapArgs, int sysMode) {
  ...
  if (!sysMode) {
    ...
    MemoryCopyUserToSystem(currentPCB, (char *)(trapArgs + 0),
                           (char *)&userformatstr, sizeof(char *));
    ...
    if (formatstr[i] == '%') {
      ...
      switch (formatstr[i]) {
        ...
        case 'c':  // chars
          ...
          if (!sysMode) {
            MemoryCopyUserToSystem(currentPCB, (char *)(trapArgs + numargs + 1),
                                   (char *)&(args[numargs]), sizeof(int));
          } else {
        ...
        case 'e':
          if (!sysMode) {
            MemoryCopyUserToSystem(currentPCB, (char *)(trapArgs + numargs + 1),
                                   (char *)&(args[numargs]), sizeof(double));
          } else {
            args[numargs] = trapArgs[numargs + 1];
        ...
        case 's':  // string
          if (!sysMode) {
            ...
            MemoryCopyUserToSystem(currentPCB, (char *)(trapArgs + numargs + 1),
                                   (char *)&userstr, sizeof(char *));
            // Now copy each individual character from that string into kernel
            // space
            for (j = 0; j < PRINTF_MAX_STRING_ARG_LENGTH; j++) {
              MemoryCopyUserToSystem(
                  currentPCB, (char *)(userstr + j),
                  (char *)&(strings_storage[string_argnum][j]), sizeof(char));
              // Check for end of user-space string
    ...
    switch (cause) {
      ...
      case TRAP_OPEN:
        ...
        if (isr & DLX_STATUS_SYSMODE) {
          ...
          MemoryCopyUserToSystem(currentPCB, (char *)trapArgs, (char *)args,
                                 sizeof(args[0]) * 2);
          MemoryCopyUserToSystem(currentPCB, (char *)(args[0]),
                                 (char *)filename, 31);
