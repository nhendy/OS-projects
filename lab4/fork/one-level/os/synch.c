...
int SemInit(Sem *sem, int count) {
  ...
  if (AQueueInit(&sem->waiting) != QUEUE_SUCCESS) {
    printf(
        "FATAL ERROR: could not initialize semaphore waiting queue in "
        "SemInit!\n");
    exitsim();
  }
  sem->count = count;
...
int SemWait(Sem *sem) {
  ...
  if (sem->count <= 0) {
    ...
    if ((l = AQueueAllocLink((void *)currentPCB)) == NULL) {
      printf(
          "FATAL ERROR: could not allocate link for semaphore queue in "
          "SemWait!\n");
      exitsim();
    }
    if (AQueueInsertLast(&sem->waiting, l) != QUEUE_SUCCESS) {
      printf(
          "FATAL ERROR: could not insert new link into semaphore waiting queue "
          "in SemWait!\n");
      exitsim();
    }
    ProcessSleep();
...
int LockInit(Lock *l) {
  ...
  if (AQueueInit(&l->waiting) != QUEUE_SUCCESS) {
    printf(
        "FATAL ERROR: could not initialize lock waiting queue in LockInit!\n");
    exitsim();
  }
  l->pid = -1;
  return SYNC_SUCCESS;
...
int LockAcquire(Lock *k) {
  ...
  if (k->pid >= 0) {  // Lock is already in use by another process
    ...
    if ((l = AQueueAllocLink((void *)currentPCB)) == NULL) {
      printf(
          "FATAL ERROR: could not allocate link for lock queue in "
          "LockAcquire!\n");
      exitsim();
    }
    if (AQueueInsertLast(&k->waiting, l) != QUEUE_SUCCESS) {
      printf(
          "FATAL ERROR: could not insert new link into lock waiting queue in "
          "LockAcquire!\n");
      exitsim();
    }
    ProcessSleep();
...
//	first checks whether the lock is a valid lock. If not, it returns
// SYNC_FAIL.
//	If the lock is a valid lock, it should check whether the calling
//	process actually holds the lock. If not it returns SYNC_FAIL. Otherwise
// it
...
int LockRelease(Lock *k) {
  ...
  if (!AQueueEmpty(&k->waiting)) {  // there is a process to wake up
    ...
    if (AQueueRemove(&l) != QUEUE_SUCCESS) {
      printf(
          "FATAL ERROR: could not remove link from lock queue in "
          "LockRelease!\n");
      exitsim();
    }
...
int CondInit(Cond *c) {
  ...
  if (AQueueInit(&c->waiting) != QUEUE_SUCCESS) {
    printf(
        "FATAL ERROR: could not initialize lock waiting queue in CondInit!\n");
    exitsim();
  }
  return SYNC_SUCCESS;
}
...
int CondWait(Cond *c) {
  ...
  if ((l = AQueueAllocLink((void *)currentPCB)) == NULL) {
    printf(
        "FATAL ERROR: could not allocate link for cond queue in CondWait!\n");
    exitsim();
  }
  if (AQueueInsertLast(&c->waiting, l) != QUEUE_SUCCESS) {
    printf(
        "FATAL ERROR: could not insert new link into cond waiting queue in "
        "CondWait!\n");
    exitsim();
  }
...
int CondSignal(Cond *c) {
  ...
  if (!AQueueEmpty(&c->waiting)) {  // there is a process to wake up
    ...
    if (AQueueRemove(&l) != QUEUE_SUCCESS) {
      printf(
          "FATAL ERROR: could not remove link from cond queue in "
          "CondSignal!\n");
      exitsim();
    }
    dbprintf('s',
             "CondSignal: Waking up PID %d, it still needs to acquire lock.\n",
             GetPidFromAddress(pcb));
    ProcessWakeup(pcb);
  } else {
    dbprintf('s',
             "CondSignal: Proc %d signalled, but no processes were waiting.\n",
             GetCurrentPid());
  }
  RestoreIntrs(intrs);
  return SYNC_SUCCESS;
...
int CondBroadcast(Cond *c) {
  ...
  if (c->lock->pid != GetCurrentPid()) {
    dbprintf('s',
             "CondBroadcast: Proc %d tried to broadcast, but it doesn't own "
             "cond %d\n",
             GetCurrentPid(), (int)(c - conds));
    return SYNC_FAIL;
  }
  ...
  dbprintf('s',
           "CondBroadcast: Proc %d successful broadcast on cond %d, still "
           "needs to release lock\n",
           GetCurrentPid(), (int)(c - conds));
  return SYNC_SUCCESS;
}
