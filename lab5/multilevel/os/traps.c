...
int TrapFileOpenHandler(uint32 *trapArgs, int sysMode) {
  ...
  if (!sysMode) {
    ...
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 0), &user_filename,
                           sizeof(uint32));
    // Argument 1: userland pointer to mode string
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 1), &user_mode,
                           sizeof(uint32));

    // Now copy userland filename string into our filename buffer
    for (i = 0; i < FILE_MAX_FILENAME_LENGTH2; i++) {
      MemoryCopyUserToSystem(currentPCB, (user_filename + i), &(filename[i]),
                             sizeof(char));
    ...
    dbprintf('F',
             "TrapFileOpenHandler: just parsed filename (%s) from trapArgs\n",
             filename);

    // Now copy userland mode string into our filename buffer
    for (i = 0; i < 10; i++) {
      MemoryCopyUserToSystem(currentPCB, (user_mode + i), &(mode[i]),
                             sizeof(char));
      // Check for end of user-space string
      if (mode[i] == '\0') break;
    }
...
int TrapFileDeleteHandler(uint32 *trapArgs, int sysMode) {
  ...
  if (!sysMode) {
    ...
    if (i == FILE_MAX_FILENAME_LENGTH2) {
      printf(
          "TrapFileDeleteHandler: length of filename longer than allowed!\n");
      GracefulExit();
    }
    dbprintf(
        'F',
        "TrapDeleteCloseHandler: just parsed filename (%s) from trapArgs\n",
        filename);
...
int TrapFileReadHandler(uint32 *trapArgs, int sysMode) {
  ...
  if (!sysMode) {
    ...
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 1), &user_mem,
                           sizeof(uint32));
    // Argument 2: integer number of bytes to read
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 2), &num_bytes,
                           sizeof(uint32));
  } else {
    // Already in kernel space, no address translation necessary
    handle = trapArgs[0];
  ...
  if (num_bytes > FILE_MAX_READWRITE_BYTES) {
    printf(
        "TrapFileReadHandler: ERROR: you cannot read more than %d bytes at a "
        "time!\n",
        num_bytes);
    return FILE_FAIL;
  }
...
int TrapFileWriteHandler(uint32 *trapArgs, int sysMode) {
  ...
  if (!sysMode) {
    ...
    if (num_bytes > FILE_MAX_READWRITE_BYTES) {
      printf(
          "TrapFileWriteHandler: ERROR: you cannot write more than %d bytes at "
          "a time!\n",
          num_bytes);
      return FILE_FAIL;
    }
...
int TrapFileSeekHandler(uint32 *trapArgs, int sysMode) {
  ...
  if (!sysMode) {
    ...
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 1), &num_bytes,
                           sizeof(uint32));
    // Argument 2: integer representing where to seek from
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 2), &from_where,
                           sizeof(uint32));
  } else {
    handle = trapArgs[0];
...
int TrapFileMkDirHandler(uint32 *trapArgs, int sysMode) {
  ...
  if (!sysMode) {
    ...
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 0), &user_filename,
                           sizeof(uint32));
    // Argument 1: userland pointer to mode string
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 1), &permissions,
                           sizeof(uint32));
    ...
    dbprintf('F',
             "TrapFileMkDirHandler: just parsed filename (%s) from trapArgs\n",
             filename);

    dbprintf(
        'F',
        "TrapFileMkDirHandler: just parsed permissions (%d) from trapArgs\n",
        permissions);
  } else {
    // Already in kernel space, no address translation necessary
    dstrncpy(filename, (char *)(trapArgs[0]), FILE_MAX_FILENAME_LENGTH2);
...
int TrapFileRmDirHandler(uint32 *trapArgs, int sysMode) {
  ...
  if (!sysMode) {
    ...
    dbprintf('F',
             "TrapFileRmDirHandler: just parsed filename (%s) from trapArgs\n",
             filename);
...
//
//----------------------------------------------------------------------
void KbdModuleInit() { *((uint32 *)DLX_KBD_INTR) = 1; }
...
static void TrapProcessCreateHandler(uint32 *trapArgs, int sysMode) {
  ...
  char name[PROCESS_MAX_NAME_LENGTH];  // Local copy of name of executable (100
                                       // chars or less)
  ...
  if (!sysMode) {
    ...
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 0), &username,
                           sizeof(char *));

    // Copy the user-space string at user-address "username" into kernel space
    for (i = 0; i < PROCESS_MAX_NAME_LENGTH; i++) {
      MemoryCopyUserToSystem(currentPCB, (username + i), &(name[i]),
                             sizeof(char));
      // Check for end of user-space string
      if (name[i] == '\0') break;
    }
    dbprintf('p',
             "TrapProcessCreateHandler: just parsed executable name (%s) from "
             "trapArgs\n",
             name);
    if (i == PROCESS_MAX_NAME_LENGTH) {
      printf(
          "TrapProcessCreateHandler: length of executable filename longer than "
          "allowed!\n");
      GracefulExit();
    }

    // Copy the program name into "allargs", since it has to be the first
    // argument (i.e. argv[0])
    allargs_position = 0;
    dstrcpy(&(allargs[allargs_position]), name);
    allargs_position += dstrlen(name) + 1;  // The "+1" is so we're pointing
                                            // just beyond the NULL

    // Rest of arguments: a series of char *'s until we hit NULL or MAX_ARGS
    for (i = 0; i < MAX_ARGS; i++) {
      // First, must copy the char * itself into kernel space in order to read
      // its value
      MemoryCopyUserToSystem(currentPCB, (trapArgs + 1 + i), &userarg,
                             sizeof(char *));
      // If this is a NULL in the set of char *'s, this is the end of the list
      if (userarg == NULL) break;
      // Store a pointer to the kernel-space location where we're copying the
      // string
      args[i] = &(allargs[allargs_position]);
      // Copy the string into the allargs, starting where we left off last time
      // through this loop
      for (j = 0; j < SIZE_ARG_BUFF; j++) {
        MemoryCopyUserToSystem(currentPCB, (userarg + j),
                               &(allargs[allargs_position]), sizeof(char));
        ...
        if (allargs_position == SIZE_ARG_BUFF) {
          printf(
              "TrapProcessCreateHandler: strlen(all arguments) > maximum "
              "length allowed!\n");
          GracefulExit();
        }
        // Check for end of user-space string
    ...
    if (i == MAX_ARGS) {
      printf(
          "TrapProcessCreateHandler: too many arguments on command line (did "
          "you forget to pass a NULL?)\n");
      GracefulExit();
    }
    numargs = i + 1;
    // Arguments are now setup
  } else {
    // Addresses are already in kernel space, so just copy into our local
    // variables
    // for simplicity
    ...
    allargs_position = 0;
    dstrcpy(&(allargs[allargs_position]), name);
    allargs_position += dstrlen(name) + 1;  // The "+1" is so that we are now
                                            // pointing just beyond the "null"
                                            // in the name
    allargs_position = 0;
    for (i = 0; i < MAX_ARGS; i++) {
...
static void TrapPrintfHandler(uint32 *trapArgs, int sysMode) {
  ...
  char *userstr;  // Holds user-space address of the string that goes with any
                  // "%s"'s
  char strings_storage[PRINTF_MAX_STRING_ARGS]
                      [PRINTF_MAX_STRING_ARG_LENGTH];  // Places to copy strings
                                                       // for "%s" into
  ...
  if (!sysMode) {
    ...
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 0), &userformatstr,
                           sizeof(char *));
    // Copy format string itself from user space to kernel space
    for (i = 0; i < PRINTF_MAX_FORMAT_LENGTH; i++) {
      MemoryCopyUserToSystem(currentPCB, (userformatstr + i), &(formatstr[i]),
                             sizeof(char));
    ...
    if (formatstr[i] == '%') {
      ...
      switch (formatstr[i]) {
        case '%':
          ...
          break;
        case 'c':  // chars
          // This argument is a single char value, but I think it's stored as a
          // full int value.
          // If it isn't, then I'm not sure how to call the *real* printf below
          // with some non-4-byte arguments.
          // The "+1" is because trapArgs[0] is the format string
          if (!sysMode) {
            MemoryCopyUserToSystem(currentPCB, (trapArgs + numargs + 1),
                                   &(args[numargs]), sizeof(int));
          } else {
        ...
        case 'l':
          ...
          i++;
        // There is intentionally no "break" here so that the %lf and %f cases
        // do the same thing.
        // This is what the previous Printf did, but I'm not sure yet that it is
        // right.
        ...
        case 'e':
          if (!sysMode) {
            MemoryCopyUserToSystem(currentPCB, (trapArgs + numargs + 1),
                                   &(args[numargs]), sizeof(double));
          } else {
            args[numargs] = trapArgs[numargs + 1];
            args[numargs + 1] = trapArgs[numargs + 2];
          }
          numargs += 2;  // numargs is incremented by 2 since a double is the
                         // size of 2 ints
          break;
        case 'd':  // integers
        case 'x':  // integers
        ...
        case 's':  // string
          if (!sysMode) {
            // First make sure we still have local string storage space
            // available
            ...
            MemoryCopyUserToSystem(currentPCB, (trapArgs + numargs + 1),
                                   &userstr, sizeof(char *));
            // Now copy each individual character from that string into kernel
            // space
            for (j = 0; j < PRINTF_MAX_STRING_ARG_LENGTH; j++) {
              MemoryCopyUserToSystem(currentPCB, (userstr + j),
                                     &(strings_storage[string_argnum][j]),
                                     sizeof(char));
              // Check for end of user-space string
            ...
            if (j == PRINTF_MAX_STRING_ARG_LENGTH) {  // String was too long!
              printf(
                  "TrapPrintfHandler: argument #%d (a string) was too long to "
                  "print!\n",
                  numargs);
              return;
            }
            // Put the address of this kernel-space string as the argument for
            // printf
          ...
          break;
        default:
          printf("TrapPrintfHandler: unrecognized format character (%c)!\n",
                 formatstr[i + 1]);
          return;
      }
    }
  ...
  switch (numargs) {
    case 0:
      ...
      break;
    case 1:
      printf(formatstr, args[0]);
      break;
    case 2:
      printf(formatstr, args[0], args[1]);
      break;
    case 3:
      printf(formatstr, args[0], args[1], args[2]);
      break;
    case 4:
      printf(formatstr, args[0], args[1], args[2], args[3]);
      break;
    case 5:
      printf(formatstr, args[0], args[1], args[2], args[3], args[4]);
      break;
    case 6:
      printf(formatstr, args[0], args[1], args[2], args[3], args[4], args[5]);
      break;
    case 7:
      printf(formatstr, args[0], args[1], args[2], args[3], args[4], args[5],
             args[6]);
    ...
    case 12:
      ...
      break;
    default:
      printf("TrapPrintfHandler: too many arguments (%d) passed!\n", numargs);
  }
...
static int TrapDiskWriteBlockHandler(uint32 *trapArgs, int sysMode) {
  ...
  if (!sysMode) {
    ...
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 0), &blocknum,
                           sizeof(uint32));
    // Argument 1: address of user-space disk_block structure
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 1), &user_block,
                           sizeof(uint32));
    ...
    blocknum = trapArgs[0];
    bcopy(
        (void *)(trapArgs[1]), (void *)&b,
        sizeof(disk_block));  // Copy message into local variable for simplicity
  }
...
//
//----------------------------------------------------------------------
void dointerrupt(unsigned int cause, unsigned int iar, unsigned int isr,
                 uint32 *trapArgs) {
  int result;
  int i;
  uint32 args[4];
    ...
    switch (cause) {
      ...
      case TRAP_OPEN:
        ...
        if (isr & DLX_STATUS_SYSMODE) {
          ...
          MemoryCopyUserToSystem(currentPCB, trapArgs, args,
                                 sizeof(args[0]) * 2);
      ...
      case TRAP_FILE_OPEN:
        ProcessSetResult(currentPCB, TrapFileOpenHandler(
                                         trapArgs, isr & DLX_STATUS_SYSMODE));
        break;
      case TRAP_FILE_CLOSE:
        ProcessSetResult(currentPCB, TrapFileCloseHandler(
                                         trapArgs, isr & DLX_STATUS_SYSMODE));
        break;
      case TRAP_FILE_DELETE:
        ProcessSetResult(currentPCB, TrapFileDeleteHandler(
                                         trapArgs, isr & DLX_STATUS_SYSMODE));
        break;
      case TRAP_FILE_READ:
        ProcessSetResult(currentPCB, TrapFileReadHandler(
                                         trapArgs, isr & DLX_STATUS_SYSMODE));
        break;
      case TRAP_FILE_WRITE:
        ...
        break;
      case TRAP_FILE_SEEK:
        ProcessSetResult(currentPCB, TrapFileSeekHandler(
                                         trapArgs, isr & DLX_STATUS_SYSMODE));
        break;
      case TRAP_FILE_MKDIR:
        ProcessSetResult(currentPCB, TrapFileMkDirHandler(
                                         trapArgs, isr & DLX_STATUS_SYSMODE));
        break;
      case TRAP_FILE_RMDIR:
        ProcessSetResult(currentPCB, TrapFileRmDirHandler(
                                         trapArgs, isr & DLX_STATUS_SYSMODE));
        break;
