...
static void TrapProcessCreateHandler(uint32 *trapArgs, int sysmode) {
  ...
  if (!sysmode) {
    ...
    for (i = 0; i < MAX_ARGS; i++) {
      ...
      // its value
      MemoryCopyUserToSystem(currentPCB, (trapArgs + 3 + i), &userarg,
                             sizeof(char *));
...
static int TrapMboxSendHandler(uint32 *trapArgs, int sysMode) {
  ...
  if (!sysMode) {
    ...
    MemoryCopyUserToSystem(currentPCB, (trapArgs + 2), &usermessage,
                           sizeof(char *));
    ...
    bcopy((void *)(trapArgs[2]), (void *)msg,
          length);  // Copy message into local variable for simplicity
  }
...
void dointerrupt(unsigned int cause, unsigned int iar, unsigned int isr,
  ...
  if (cause & TRAP_TRAP_INSTR) {
    ...
    switch (cause) {
      ...
      case TRAP_YIELD:
        ProcessYield();
        ProcessSchedule();  // this just moves the item on front of the queue to
                            // the back
        ClkResetProcess();
        break;
