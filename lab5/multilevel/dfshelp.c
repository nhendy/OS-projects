...
-You can copy as much code from lab5/flat as you want, including
lab5/flat/apps/fdisk but make sure that you
make the necessary modifications.

-You can (optionally) do without a filename field in the inode since that
information is stored in each directory entry.
...
-Remember that directories are like files, so you can manipulate them using file
open, read, and
write functions.
...
/*HELPER DEFINITIONS
//These are definitions which you MAY use. These are optional and have been
randomly grouped together here.
...
//They may not necessarily fit in directly with the given code. You can copy the
logic and implement
//your own exact ones.
...
//	convertModeToInt
//
//	Converts a mode string to integer. Mode "r" is 1, mode "w" is 2. The
various
...
inode_t MakeInodeFromPath(char *path, int mode, int type)
{
  ...
  //it may insert the inode number and file or directory name in global
variables
}
