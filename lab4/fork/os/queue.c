...
static Link linkpool[QUEUE_MAX_LINKS];  // Memory space for each link, since we
                                        // can't use malloc()
...
///////////////////////////////////////////////////////
void testfuncvoid() { dbprintf('q', "testfunvoid\n"); }
int testfuncint() {
...
int AQueueModuleInit() {
  ...
  if (AQueueInit(&freeLinks) != QUEUE_SUCCESS) {
    printf(
        "FATAL ERROR: could not initialize freeLinks queue in "
        "AQueueModuleInit!\n");
    exitsim();
  }
  ...
  for (i = 0; i < QUEUE_MAX_LINKS; i++) {
    ...
    if (AQueueInsertLast(&freeLinks, &(linkpool[i])) != QUEUE_SUCCESS) {
      printf(
          "FATAL ERROR: could not insert link into freeLinks in "
          "AQueueModuleInit!\n");  // Adds structure to global queue of free
                                   // links
      exitsim();
    }
  }
...
int AQueueMoveAfter(Queue *q, Link *after, Link *l) {
  ...
  // Now update l's prev and next pointers to put l into the list after the link
  // "after"
  l->prev = after;
  l->next = after->next;
