...
void main(int argc, char *argv[]) {
  ...
  num_r1 = num_s2;
  num_r2 = num_co / 4;
  num_r3 = mistd::min((int)(num_s2 * 2), (int)(num_co / 4));
  ...
  if (sem_wait(sem) == SYNC_FAIL) {
    LOG("bad semaphore in %d", argv[0]);
    Exit();
  }
  if (mbox_close(s2) == MBOX_FAIL) {
    LOG("Error in closing s2 mbox %d", argv[0]);
    Exit();
  }
  if (mbox_close(co) == MBOX_FAIL) {
    LOG("Error in closing co mbox %d", argv[0]);
    Exit();
  }
  if (mbox_close(s) == MBOX_FAIL) {
    LOG("Error in closing s mbox %d", argv[0]);
    Exit();
  }
  if (mbox_close(o2) == MBOX_FAIL) {
    LOG("Error in closing o2 mbox %d", argv[0]);
    Exit();
  }
  if (mbox_close(so4) == MBOX_FAIL) {
    LOG("Error in closing so4 mbox %d", argv[0]);
    Exit();
  }
  if (mbox_close(c2) == MBOX_FAIL) {
    LOG("Error in closing c2 mbox %d", argv[0]);
    Exit();
  }
