...
void ProcessDecayAllEstcpusAndRecalcPriorities() {
  int i;
  Link *l;
  PCB *pcb;
  dbprintf('k', "Decay all est cpus\n");
  for (i = 0; i < NUM_PRIORITY_QUEUES; ++i) {
    if (AQueueEmpty(&runQueue[i])) continue;
...
int CountNonIdleProcesses(Queue *runQueue) {
  ...
  while (l) {
    pcb = AQueueObject(l);
    if ((pcb->flags & PROCESS_TYPE_IDLE) == 0) count++;
    l = AQueueNext(l);
  }
  return count;
}
...
void PrintQueue(Queue *queue) {
  ...
  while (l) {
    pcb = AQueueObject(l);
    dbprintf('k', "PID [%d, %d] ", GetPidFromAddress(pcb), pcb->priority);
    l = AQueueNext(l);
  }
  dbprintf('k', "\n");
}
...
void ProcessRelocate(PCB *pcb) {
  ...
  if ((pcb->l = AQueueAllocLink(pcb)) == NULL) {
    printf("FATAL ERROR: could not allocate link in ProcessRelocate!\n");
    exitsim();
  }
  ProcessInsertRunning(pcb);
}
...
void ProcessSchedule() {
  ...
  // Only runnable processes get their estcpu decayed
  if ((currentPCB->flags &
       (PROCESS_STATUS_RUNNABLE | PROCESS_STATUS_YIELDING)) ==
      PROCESS_STATUS_RUNNABLE) {
...
void ProcessWakeup(PCB *wakeup) {
  ...
  wakeup->stats.time_to_sleep = 0;
  wakeup->stats.sleep_timestamp = 0;
...
int ProcessInsertRunning(PCB *pcb) {
  ...
  dbprintf('k', "ProcessInsertRunning\n");
  if ((pcb->flags & PROCESS_STATUS_RUNNABLE) != PROCESS_STATUS_RUNNABLE) {
    printf(
        "FATAL ERROR: process must be RUNNABLE before inserting into a "
        "runQueue\n");
    exitsim();
  }
...
uint32 get_argument(char *string) {
  ...
  if (str[location] == '\0' || location >= 99) return 0;
