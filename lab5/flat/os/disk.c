...
int DiskBytesPerBlock() { return DISK_BLOCKSIZE; }

//----------------------------------------------------------------------------
// DiskSize returns the size of the hard disk, in bytes.
//----------------------------------------------------------------------------

int DiskSize() { return DISK_BLOCKSIZE * DISK_NUMBLOCKS; }
...
int DiskCreate() {
  ...
  if (filename[11] == 'X') {
    printf(
        ...
        "include/os/disk.h.  Cowardly refusing to do anything.\n");
    GracefulExit();
  }
  ...
  // You need to do this because the writeblock/readblock operations are allowed
  // in
  // random order.
...
int DiskWriteBlock(uint32 blocknum, disk_block *b) {
  ...
  if (blocknum >= DISK_NUMBLOCKS) {
    printf(
        "DiskWriteBlock: cannot write to block larger than filesystem size\n");
    return DISK_FAIL;
  }
  ...
  if (filename[11] == 'X') {
    printf(
        "DiskWriteBlock: you didn't change the filesystem filename in "
        "include/os/disk.h.  Cowardly refusing to do anything.\n");
    GracefulExit();
  }
...
int DiskReadBlock(uint32 blocknum, disk_block *b) {
  ...
  if (blocknum >= DISK_NUMBLOCKS) {
    printf(
        "DiskReadBlock: cannot read from block larger than filesystem size\n");
    return DISK_FAIL;
  }

  if (filename[11] == 'X') {
    printf(
        "DiskReadBlock: you didn't change the filesystem filename in "
        "include/os/disk.h.  Cowardly refusing to do anything.\n");
    GracefulExit();
  }
