...
typedef struct PCB {
  ...
  int autowake;  // Flag to indicate that ProcessSchedule can wake this up after
                 // wakeuptime is reached
  int isyielding;  // Indicates that a PCB is yielding, so don't increment it's
                   // estcpu
...
#define PROCESS_NUM_QUEUES \
  ((PROCESS_MAX_PRIORITY + 1) / PROCESS_PRIORITIES_PER_QUEUE)  // should be 32
