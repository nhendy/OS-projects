...
typedef struct PCB {
  ...
  uint32 pagetable[ADDRESS_TO_PAGE(MAX_VIRTUAL_ADDRESS) + 1];  // Statically
                                                               // allocated
                                                               // page table
  HeapNode heap[HEAP_NUM_NODES];
...
#define PROCESS_STACK_USER_STACKPOINTER \
  (PROCESS_STACK_IREG + 29)          // r29 is user stack pointer
